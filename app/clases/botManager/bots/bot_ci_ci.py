...
class botCiCi(taskSeqManager):
    ...
    def get_volume_ci1_ci2(self, size, price, price2):
        self.log.info(
            f"entrando a get_volume_ci1_ci2: {size}, {price}, {price2} ")
                ...
                if verificarOperar["puedoOperar"] == True:
                    ...
                    if sideBook == "BI":
                        if posicion48h < volume_limit_CI and posicion48h > 0:
                            ...
                            self.log.info(
                                "no tengo suficiente size en ci2h pero tengo algo entonces modifico el size")
                    else:
    ...
    async def operar_orden_contraria(self, orden, symbolCheck, sideCheck, id_order, sideOrder):
        ...
        try:
            ...
            if self.botData["market"] == True:
                self.log.info(
                    f"esta market mando a crear orden nueva y cancelar orden haberla en 2 hilos ")
                ...
                self.log.info(
                    f"llegaron respuestas, ordennew: {ordenNew}, cancelarOrdenhaberla: {cancelarOrdenhaberla}")
                response = ordenNew
            else:
