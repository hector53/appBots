...
class botCiCi(taskSeqManager):
        ...
        try:
         ...
         #   self.bb_ci.append(bb_ci_actual)
          #  self.bb_48.append(bb_48_actual)
            ...
            latest_limit_asset_price_48h_BID = price_ci_bi * (lower +1)
            latest_limit_asset_price_48h_ASK = price_ci_of * (upper + 1)
            bid_estrategia = (price_48_bi - price_ci_of) /  price_ci_of
            ask_estrategia = (price_48_of - price_ci_bi) / price_ci_bi
            """
    ...
    async def run_forever_bb(self):
        try:
         ...
         #       await self.pausedBB.wait()
            ...
            self.log.warning(                f"saliendo del ciclo run foreverBB del botBB con id: {self.id}")
        ...
        try:
            ...
            if verificarOrdenCreada["status"] == True:  # si tengo orden creada
                ...
                if verificarOperar["puedoOperar"] == True:
                    ...
                    asset_price_CI2 = self._tickers[self.botData["byma48h"]
                                                     ][sideBook][0]["price"]
                    if verificarOperarCI2["primeraOrden"] == True:
                        asset_price_CI1 = self._tickers[self.botData["byma48h"]
                                                     ][sideBook][1]["price"]
        ...
        try:
            ...
            if verificarOrdenCreada["status"] == True:  # si tengo orden creada
                ...
                if verificarOperar["puedoOperar"] == True:
                    ...
                    asset_price_CI1 = self._tickers[self.botData["bymaCI"]
                                                     ][sideBook][0]["price"]
                    if verificarOperarCI["primeraOrden"] == True:
                        asset_price_CI1 = self._tickers[self.botData["byma48h"]
                                                     ][sideBook][1]["price"]
                    # size del book q puedo tomar sus valores
    ...
    def update_limits(self, symbol, price, sideBook):
        ...
        try:
            ...
            dataMd = {"type": "puntas", "symbolTicker": self.botData["bymaCI"]}
            self.fix.ws.send(str(dataMd))
        except Exception as e:
    ...
    async def operar_orden_contraria(self, orden, symbolCheck, sideCheck, id_order, sideOrder):
        ...
        try:
            ...
            size = orden["lastQty"]
            asset_price_CON = self._tickers[symbolCheck][sideCheck][0]["price"]
            if verifyF["primeraOrden"] == True:
            ...
            if self.botData["market"] == True:
                ...
                task1 = asyncio.create_task(self.clientR.nueva_orden(
