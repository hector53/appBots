...
class botTriangulo(taskSeqManager):
        ...
        try:
            ...
            if verificarOrdenCreada["status"]==True:
                ...
                if verificarFuturo2["puedoOperar"]==True and verificarPase["puedoOperar"]==True:
                    ...
                    if calcularLimit>0:
                        ...
                        #modificar orden
                            ...
                            self.log.warning("es lo mismo asi q no actuaklizo ")
                else:
                ...
                if verificarFuturo2["puedoOperar"]==True and verificarPase["puedoOperar"]==True:
                    ...
                    self.log.warning("no puedo crearla xq no hay futuro o pase necesario o estoy de primero en el otro futuro, pero como no tengo nada en este , entonces no hago nada, xq sino es un ciclo infinito")
        ...
        try: 
            ...
            if verificarOrdenCreada["status"]==True:
                ...
                if verificarFuturo2["puedoOperar"]==True and verificarPase["puedoOperar"]==True:
                    ...
                    self.log.warning("debo borrar xq no hay futuro2 o pase ask pero no hago xq elegi aqui no hacer nada ")
            else: 
                ##self.log.info("no tengo orden la creo")
                #no tengo orden la creo 
                verificarFuturo1 = True
    ...
    async def  verificar_futuro2_ask(self): 
        ...
        try:
            ...
            #self.log.info(f"verificarOrdenCreada {verificarOrdenCreada}")
            if verificarOrdenCreada["status"]==True:
                #self.log.info("tengo orden creada")
                orden =  verificarOrdenCreada["data"]
                #self.log.info(f"orden {orden}")
                #self.log.info(f"price {orden['price']}")
                #es true osea q si tengo orden 
    ...
    async def run_forever(self):
        try:
            ...
            self.log.warning(                f"saliendo del ciclo run forever del botBB con id: {self.id}")
    ...
    async def  verificar_triangulo_pase(self, i, symbolNueva, sideNueva, typeOrder, details, clOrderID):
        ...
        if self.botData["pegados"][i]["ordenPegada"]["symbol"]==self.botData["paseFuturos"] and self.botData["pegados"][i]["ordenPegada"]["side"]=="Buy":
            ...
            if self._tickers[self.botData["paseFuturos"]]["BI"][0]["price"]==self.botData["pegados"][i]["ordenPegada"]["price"]:
                ...
                self.log.warning("no estoy de primero en el book entonces no me sirve para cerrar la orden vieja ")
