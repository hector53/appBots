...
class BotsController:
    ...
    async def start_bot_new():
      ...
      #  print("startBot",req_obj)
        ...
        if getFixTask:
            ...
            #log.logInfo(f"si existe el bot en la db asi q continuo")
            ...
            if getBotEjecutando["status"] == 0:
                #log.logInfo(f"el bot esta desactivado asi q lo inicio")
                opciones = getBotEjecutando["opciones"]
                ...
                #log.logInfo(f"el bot esta en otro estado asi q lo actualizo")
                # activar bot
    ...
    async def cancel_order_manual():
        from app import fixM
        req_obj = request.get_json()
        #log.logInfo(f"cancel_order_manual BOT: {req_obj}")
        x = req_obj["order"]
        fix=req_obj["fix"]
    ...
    async def detener_bot():
        req_obj=request.get_json()
        #log.logInfo(f"DETENER BOT: {req_obj}")
        id_bot=req_obj["id"]
        fix=req_obj["fix"]
        response={"status": True}
        ...
        if fix["active"] == 1:
            ...
            #log.logInfo("fix no esta activa, entonces actualizo solo en db ")
            await DbUtils.update_status_bot_ejecuntadose(id_bot, 0)
            response={"status": True}
    ...
    def bot_data_charts(id):
       ...
       # print(req_obj)
        try:
            ...
            if bot:
                bot['_id']=str(bot['_id'])
                #  #log.logInfo("ahora a crear el bot ejecutando ")
            ...
            if statusBot == 0:
                ...
                if botE["type_bot"] == 0:
                  #  #log.logInfo("es triangulo")
                    ...
                    log.logInfo(f"tickers en bot : {fixM.main_tasks[fix['user']].botManager.main_tasks[botE_id]._tickers}")
